...
interface Option<T extends string> {
  ...
  /** Optional dimmed second line under the label (status, host, hint …). */
  description?: string;
}
    ...
    isOpen && menuPosition && typeof document !== "undefined"
      ? createPortal(
          <div
            ref={menuRef}
            role="listbox"
            className="fixed z-[10050] overflow-hidden rounded-xl border border-border/60 bg-popover shadow-xl shadow-black/[0.08] animate-slide-in-up"
            style={{
              ...
              {options.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full px-3.5 py-2.5 text-start flex items-center justify-between gap-2 text-sm transition-all duration-150",
                      isSelected
            ...
            {footerAction && (
              <div className="border-t border-border/50 p-1.5">
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-start text-sm font-medium text-foreground transition-colors hover:bg-accent/50"
                >
                  {footerAction.icon}
                  {footerAction.label}
                </button>
